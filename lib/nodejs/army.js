...
module.exports = {
    mailWrite: async function (soldierName, articles, writeablePeriodMax) {
        ...
        console.log(Object.keys(articles).length + '개의 기사를 ' + soldierName + ' 훈련병에게 전송합니다. ')
        ...
        for (i = 0; i < $('.cafe-card-box').length; i++) {
            if ($('.cafe-card-box').eq(i).find('.id').text().trim().split(' ')[0] === soldierName) {
                if ($('.cafe-card-box').eq(0).find('.btn-wrap').find('.btn-green').text() === '위문편지') {
                    console.log( soldierName + ' 훈련병의 카페로 이동합니다.')
        ...
        if (writeablePeriodMax === 1) {
            console.log(soldierName + ' 훈련병의 WPM 이 크롤되지 않았습니다. 크롤링합니다.')
            var $ = cheerio.load(await page.content());
        ...
        for (i = 0; i < Object.keys(articles).length; i++) {
            ...
            console.log('작성중입니다 : ' + articles[i].title)
        ...
        console.log( soldierName + ' 훈련병에게 편지를 다 보냈습니다. 종료합니다.')
    ...
    enrollSoldier: async function (soldierArray) {
        ...
        for (arrayIndex in soldierArray) {
            var soldierName = arrayIndex;
            var birthInfo = soldierArray[arrayIndex]['birth'];
            var joinTrainUnit = soldierArray[arrayIndex]['joinTrainUnit'];
            var joinDate = soldierArray[arrayIndex]['joinDate'];

		  console.log(`${soldierName} ${birthInfo} 
		  ${joinTrainUnit} ${joinDate}`);
            await page.goto('https://www.thecamp.or.kr/missSoldier/viewMissSoldierSearch.do');
