let error_msg = ['이름이 정확하지 않습니다.', '생년월일이 정확하지 않습니다.', ['입영일자가 정확하지 않습니다.', '입영일자가 현재 시간 기준 30일을 초과했습니다.'], ['편지 장수를 설정하지 않았습니다.', '남은 편지가 있습니다.'], '군 세부정보가 입력되지 않았습니다.', ['재입력한 비밀번호가 옳지 않습니다.', '비밀번호의 길이 제한이 지켜지지 않았습니다']]
    ...
    switch (option) {
        ...
        case 5:
            ...
            if (($(id).val().match(/[0-9]/gi) != null) && $(id).val().match(/[0-9]/gi).length === $(id).val().length && $(id).val().length === 8) {
                ...
                let enterdate = new Date(dateStr);
                let timedif = enterdate - curdate;
                let month = 2592000000 * 2;
                if (timedif <= 0 && month * (-1) <= timedif) {
                    flag = 1;
                    error_arr[2] = 0;
                }
                else {
